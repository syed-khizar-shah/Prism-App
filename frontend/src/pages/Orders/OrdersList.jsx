...
export default function OrdersList() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    // Optimistically update local state — no full refetch, no loading flash
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {total > 0 ? `${total} total orders` : 'View, manage, and track customer orders'}
          </p>
        </div>
        ...
        >
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search orders..."
              className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400 transition-colors"
            />
          </div>
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value)}
            className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400 transition-colors"
          >
            <option value="">All statuses</option>
            <option value="pending">Pending</option>
            <option value="confirmed">Confirmed</option>
                    ...
                    <tr key={order._id} className="hover:bg-gray-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-medium text-gray-900">{order.customer?.name || 'N/A'}</div>
                            <div className="text-xs text-gray-500">{order.customer?.email || order.customer?.phone || 'No contact'}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select
                          value={order.status}
                          onChange={(e) => handleStatusChange(order._id, e.target.value)}
                          className={`px-2.5 py-1 text-xs font-medium rounded-full ring-1 ring-inset border-0 cursor-pointer ${STATUS_STYLES[order.status] || STATUS_STYLES.pending} focus:outline-none focus:ring-2 focus:ring-gray-900/20 transition-colors`}
                        >
                          {['pending', 'confirmed', 'processing', 'completed', 'cancelled', 'refunded'].map(s => (
                            <option key={s} value={s}>{s}</option>
                          ))}
            ...
            <div className="flex items-center justify-between px-6 py-3 border-t border-gray-100 bg-gray-50/50">
              ...
              <div className="flex gap-2">
                <button
                  disabled={page <= 1}
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  className="inline-flex items-center gap-1 px-3 py-1.5 border border-gray-200 bg-white rounded-md text-xs font-medium disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors"
                >
